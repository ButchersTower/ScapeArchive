...
public class PathFind {
	...
	public PathFind(int tx, int ty, int id, int px, int py) {
		this.tx = tx;
		this.ty = ty;
		this.id = id;
		this.px = px;
	...
	public void getAdj(int x, int y, int f) {
		...
		int xx = d[0] - 1;
		int yy = d[1] - 1;
		if (Panel.getMapVar(xx, yy) != 3 && Panel.getMapVar(xx, yy) != -1) {
	...
	// Draws an arrow to parent
	void arrowParent() {
		for (int i = 0; i < closed.size(); i++) {
			if (closed.get(i)[0] < closed.get(i)[4]) {
				// player is less than (west 0f) parent
				// left
				// arrow facing east
				if (closed.get(i)[1] < closed.get(i)[5]) {
					// player y is less than (north of) parent
					// arrow facing down
					Panel.g1.drawImage(ImgLoad.arrows[7],
							closed.get(i)[0] * 32 + 12, closed.get(i)[1] * 32,
							null);
				} else if (closed.get(i)[1] == closed.get(i)[5]) {
					// even
					Panel.g1.drawImage(ImgLoad.arrows[4],
							closed.get(i)[0] * 32 + 12, closed.get(i)[1] * 32,
							null);
				} else if (closed.get(i)[1] > closed.get(i)[5]) {
					// down
					Panel.g1.drawImage(ImgLoad.arrows[2],
							closed.get(i)[0] * 32 + 12, closed.get(i)[1] * 32,
							null);

				}
			} else if (closed.get(i)[0] == closed.get(i)[4]) {
				// even on x axis
				// even
				if (closed.get(i)[1] < closed.get(i)[5]) {
					// player y is less than (north of )parent
					// up
					// arrow facing south
					Panel.g1.drawImage(ImgLoad.arrows[6],
							closed.get(i)[0] * 32 + 12, closed.get(i)[1] * 32,
							null);
				} else if (closed.get(i)[1] == closed.get(i)[5]) {
					// even
					...
					// down
					Panel.g1.drawImage(ImgLoad.arrows[1],
							closed.get(i)[0] * 32 + 12, closed.get(i)[1] * 32,
							null);
				}
			} else if (closed.get(i)[0] > closed.get(i)[4]) {
				// player is greater than (east of) parent
				// arrow facing west
				if (closed.get(i)[1] < closed.get(i)[5]) {
					// up
					Panel.g1.drawImage(ImgLoad.arrows[5],
							closed.get(i)[0] * 32 + 12, closed.get(i)[1] * 32,
							null);
				} else if (closed.get(i)[1] == closed.get(i)[5]) {
					// even
					Panel.g1.drawImage(ImgLoad.arrows[3],
							closed.get(i)[0] * 32 + 12, closed.get(i)[1] * 32,
							null);
				} else if (closed.get(i)[1] > closed.get(i)[5]) {
					// down
					Panel.g1.drawImage(ImgLoad.arrows[0],
							closed.get(i)[0] * 32 + 12, closed.get(i)[1] * 32,
							null);
				}
			}
